...
model = dict(
    ...
    backbone=dict(
        ...
        dw_kss=[5, 5, 5, 5], dim_heads=[16, 32, 32, 32], window_sizes=[10, 10, 10, 10],
        hybrid_eopss=[[0], [0], [3], [3]],
        conv_kss=[1, 1, 1, 1], conv_groupss=[1, 1, 1, 1],
        qkv_bias=True, attn_drop=0., drop=0., drop_path=0.1,
        v_group=False, attn_pre=False, ls_value=1e-6,
        sync_bn=False, out_indices=(3, 4), pretrained='../pretrained/emo2_20M.pth', frozen_stages=-1, norm_eval=True,),
    neck=dict(
        type='SSDNeck',
        in_channels=(320, 448),
        out_channels=(320, 448, 512, 256, 256, 128),
    ...
    bbox_head=dict(
        type='SSDHead',
        in_channels=(320, 448, 512, 256, 256, 128),
        num_classes=80,
        use_depthwise=True,
        norm_cfg=dict(type='BN', eps=0.001, momentum=0.03),
        act_cfg=dict(type='ReLU6'),
